...
% Calculate the size of the previous sample
    OLD_Iteration_size=length(mu_back)                                     ;
...
% Pre calculation
AAA(1,:) = 1:3:3*Num_part*Species_to_learn                                ;
AAA(2,:) = 2:3:3*Num_part*Species_to_learn                                ;
AAA(3,:) = 3:3:3*Num_part*Species_to_learn                                ;
...
i = i+1                                                                   ;
    ...
    % Pre-calculation of the background emission rates times data acquisition time (Time)
    I_b          =  mu_back(i-1) * Time{1}                                ;

    for mm=1:Species_to_learn
        
        % Pre-calculation of the molecular brightness times data acquisition time (Time)
        I            =  mu(mm,i-1)      * Time{1}                                ; 

        % Pre-calculation of the variance of the kinetics at each time step and coordiante
        vdif{1}      =   2*D(mm,i-1)*[0 Time{1}(2:end)]./([wxy ; wxy ; wz].^2)  ; 
        
    % Start the FOR loop on the number of molecules
        for num_par=1:Num_part
        
            % Check if the molecule num_par is active (b=1)
            if  b(1,(mm-1)*Num_part+num_par,i-1)==1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Sampling ACTIVE trajectories for particle num_par in  imported signal k %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            x{1}  = FFBS( observation{1} , I             , I_b        , vdif{1}      , ...
                          x{1}           , num_par       , mu_prior   , var_prior    , ...
                          b(1,:,i-1)     , Trace_size(1) , PSF_Func   , Filter_typ   , ...
                          yy             , pp            , f_3DG      , f_2DGL_D1_2  , ...
                          f_2DGL_D3      , f_2DGC        , AAA        , mm           , ...
                          Num_part      );
            end
        end
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    x{1}  = Inactive_molecules( x{1}      , Trace_size(1) , Time{1}          , b(1,:,i-1) , ...
                                mu_prior  , var_prior     , wxy              , wz         , ...
                                D(:,i-1)  , Num_part      , Species_to_learn );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    xx=x{1};
       ...
       switch  PSF_Func
            case 1 % 3D Gaussian
                  Constant{1} =exp(-2*((xx(AAA(1,:),:).^2)+...
                                       (xx(AAA(2,:),:).^2)+...
                                       (xx(AAA(3,:),:).^2)));
                               
                  for l=1:len_concen_radius
          
                      concentration(1,i-(floor((i-1)/10000)*10000),1:Trace_size(1),l) =...
                          ...
                      b(1,:,i-1)*(sqrt( ((xx(AAA(1,:),:).^2)+...
                                         (xx(AAA(2,:),:).^2)+...
                                         (xx(AAA(3,:),:).^2)))<=concen_radious(l));
                  end 
                  
            case 2 % 2D Gaussian-Lorentzian
                  Constant{1} = exp(-2*((xx(AAA(1,:),:).^2)+(xx(AAA(2,:),:).^2))./...
                                                         (1+(xx(AAA(3,:),:).^2)))./...
                                                         (1+(xx(AAA(3,:),:).^2));
                  ...
                  for l=1:len_concen_radius
                      concentration(1,i-(floor((i-1)/10000)*10000),1:Trace_size(1),l) =...
                          ...
                      b(1,:,i-1)*(sqrt( 0.5.*log(1+(xx(AAA(3,:),:).^2))+...
                                                  ((xx(AAA(1,:),:).^2)+...
                                                   (xx(AAA(2,:),:).^2))./...
...
% We will use it to sample the diffusion coefficient
   ...
   kk_diffusion=sum((repmat([wxy , wxy , wz].^2,1,Num_part)*...
((xx(3*((mm-1)*Num_part)+1:3*mm*Num_part,2:end-1)-xx(3*((mm-1)*Num_part)+1:3*mm*Num_part,3:end)).^2))./(Time{1}(3:end)));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%% Sample the diffusion coefficient %%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  D(mm,i)   =    1/gamrnd(D_alpha_prior+(((Trace_size(1)-2))*1.5*Num_part) , ...
...
% them several times to have a better mixture.
       ...
       for j=1:iter_b_mu
           ...
           Time          , b(1,:,i)            , mu_back_proposal , mu_back_alpha  , ...
           mu_back_beta  , Constant            , Species_to_learn , Num_part);
             
           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           %%%%%%%%%%%%%%% Update the weight over loads (hyper priors) %%%%%%%%%%%%%%%
           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           q(1,:,i) = betarnd(alpha_q/Num_part+b(:,:,i),(beta_q*(Num_part-1)/Num_part)+1-b(1,:,i));
          
           
           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           %%%%%%%%%%%%%%%%%%%%%%%%%% Update the loads  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           [ b(1,:,i) , acceptance_rate_b ] = Sample_Loads( ...
                ...
                ...
           b(1,:,i)     , mu(:,i)   , observation{1} , acceptance_rate_b  , ...
           Time{1}      , Num_part  , mu_back(i)     , q(1,:,i)           , ...
           Constant{1}  , Species_to_learn );
       end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
       ...
       if find((OLD_Iteration_size+floor((1:1:save_size).*Max_iter./save_size))==i)>=1
            ...
            if GUI==1              
                 ...
                 plot(handles.axes15,1:1:i,sum(reshape(b(1,:,:),size(b,2),size(b,3)),1))
               ...
               if  Filter_typ ==1
                 ...
                 histogram(handles.axes18,sum(reshape(b(1,:,1:i),size(b,2),size(b,3)),1),...
...
acceptance_rate_b   = acceptance_rate_b  / ((i-length(D)-1)*iter_b_mu)  ;
